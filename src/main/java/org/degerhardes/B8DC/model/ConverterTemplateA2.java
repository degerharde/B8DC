...
import static org.degerhardes.B8DC.model.TemplateStaticMethods.getBits;

public class ConverterTemplateA2 implements ConverterTemplate {
    @Override
    public String convert(String[] arr) {
        ...
        sb.append(arr[++curPos]).append(" ").append(arr[++curPos]).append(" - время аварии (чч:мм): ")
                .append(convertToDec(arr[curPos])).append(":").append(convertToDec(arr[curPos-1])).append("\n")
                .append(arr[++curPos]).append(" ").append(arr[++curPos]).append(" - CRC 1, 2");
        return sb.toString();
    }
}
